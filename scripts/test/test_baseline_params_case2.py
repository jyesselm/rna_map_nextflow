#!/usr/bin/env python3
"""Test baseline parameters on case_2 data.

This script tests the original baseline Bowtie2 parameters on case_2
to see what the alignment rate was before optimization.
"""
...
log = get_logger("TEST_BASELINE_PARAMS_CASE2")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Test baseline parameters on case_2 data"
    )
    parser.add_argument(
        "--fasta",
        type=Path,
    ...
    parser.add_argument(
        "--params-file",
        type=Path,
        default=Path("original_baseline_parameters.txt"),
        help="File with baseline parameters",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("case2_baseline_test_results"),
    ...
    with open(args.params_file) as f:
        ...
        params_lines = [
            line
            for line in params_line.split("\n")
            if line.strip() and not line.strip().startswith("#")
        ]
        if not params_lines:
            print(f"ERROR: No parameters found in {args.params_file}")
            return 1
    ...
    print("=" * 80)
    print("Testing Baseline Parameters on Case 2")
    print("=" * 80)
    ...
    print(f"Parameters: {' '.join(bt2_args)}")
    print()
