...
if [ -n "$CONTAINER_CMD" ]; then
    ...
    echo ""
    echo "To convert on the cluster:"
    echo "  1. Save Docker image: docker save $DOCKER_IMAGE | gzip > rna-map-optimization.tar.gz"
    echo "  2. Transfer to cluster: scp rna-map-optimization.tar.gz user@cluster:/path/"
    echo "  3. On cluster, load image: docker load < rna-map-optimization.tar.gz"
    echo "  4. Convert to Apptainer: apptainer build rna-map-optimization.sif docker-daemon://${DOCKER_IMAGE}"
    exit 0
fi
